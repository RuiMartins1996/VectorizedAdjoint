...
!~~~> use the RKINT_TLM and RKINT_SOA integrators
 ...
 SUBROUTINE RKINT_SOADR(N, ADY, ADJ_RHS, JACV,                      &
           AD2Y, SOA_RHS, RHS, TLM_RHS,                             &
           P, W2, QUAD2_RHS, D2Y0DP2, DP,                           &
           Tstart, Tend, Nc,                                        &
           FwdAbsTol, FwdRelTol,                                    &
           AdjAbsTol, AdjRelTol,                                    &
...
!
! RKINT_SOADR manages checkpointing by performing all the necessary
...
!
!      W2 (P by 1 array, double precision) = quadrature ODE system state at (T->Tend)
!
...
!                  F(1) = .... etc.
!
!     FwdRelTol (2*N by 1 array, double precision),
!     FwdAbsTol (2*N by 1 array, double precision) :
...
!                                  be performed.
!                       FwdRelTol(1:N),FwdAbsTol(1:N) = relative/absolute tolerances for
!                                     the forward model variables
!                       FwdRelTol(N+1:2*N),FwdAbsTol(N+1:2*N) = relative/absolute tolerances for
!                                     the tangent linear model variables
!
!     AdjRelTol (2*N+P by 1 array, double precision),
!     AdjAbsTol (2*N+P by 1 array, double precision)
...
!                       AdjRelTol(N+1:2*N),AdjAbsTol(N+1:2*N) = relative/absolute tolerances for
!                                     the second order adjoint model variables (ADY)
!                       AdjRelTol(2*N+1:P),AdjAbsTol(2*N+1:P) = relative/absolute tolerances for
!                                     the quadrature variables (W2)
!
...
!~~~>     OUTPUT ARGUMENTS:
!
!     AD2Y(N)    -> vector of final second order adjoint states (at T->Tstart)
!     W2(P)      -> Hessian-vector product (d2G/dp2)*DP
!
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   EXTERNAL D2Y0DP2 !final correction to W2
   INTEGER,          INTENT(INOUT) :: N, Nc
   INTEGER,          INTENT(INOUT) :: P
   DOUBLE PRECISION, INTENT(INOUT) :: ADY(N), AD2Y(N)
   ...
   DOUBLE PRECISION, INTENT(INOUT) :: AdjAbsTol(2*N+P), AdjRelTol(2*N+P)
...
!~~~> Forward integration status parameters:
   DOUBLE PRECISION :: FwdISTATUS_T(20)
!~~~> Adjoint integration status parameters:
...
!               = 0 otherwise
   ...
   IF (.NOT. Checkpointing) THEN
       ...
       CALL RKINT_SOA(N, ADY, ADJ_RHS, JACV,                    &
                      AD2Y, SOA_RHS,                            &
                      P, W2, QUAD2_RHS, DP,                     &
                      Tstart, Tend,                             &
                      AdjAbsTol, AdjRelTol,                     &
   ...
   IF (AdjICNTRL(5) .EQ. 1) THEN
       ...
       CALL D2Y0DP2(N,P,DP,Tstart,ADY,AD2Y,W2update)

       CALL WAXPY(P,1.d0,W2update,1,W2,1)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   IERR = Code
   PRINT * , &
     'Forced exit from RKINT_SOADR due to the following error:' 
 ...
 END SUBROUTINE RKINT_SOADR


END MODULE RKSOADR_MODULE