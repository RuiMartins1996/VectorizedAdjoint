...
!~~~> Use the RKINT and RKINT_ADJ subroutines for model integrations
 ...
 SUBROUTINE RKINT_ADJDR_M(N, M, ADYM, ADJ_RHS, JACV,                 &
           P, WM, QUAD_RHS, DY0DP, RHS,                              &
           Tstart, Tend, Nc,                                        &
           FwdAbsTol, FwdRelTol,                                    &
           AdjAbsTol, AdjRelTol,                                    &
...
! Solves M adjoint systems, reusing the same forward integration data (at most one
! forward model integration is performed, if needed).
!
! RKINT_ADJDR_M manages checkpointing by performing all the necessary
! checkpoint restores and forward trajectory recomputations.
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!
!      N (integer)       = size of the adjoint ODE system
!
!      M (integer)       = size of the adjoint ODE system
!
!      ADYM (N by M array, double precision)  = vector of adjoint final conditions
!                                              for the M adjoint systems (at T=Tend)
!
...
!
!      WM (P by M array, double precision) = quadrature ODE system state at T->Tend for
!                                            the M systems
!
!      Tstart (double precision), Tend (double precision) :
!          [Tstart,Tend]  = time range of integration. RKINT_ADJDR_M requires that Tstart<=Tend
!
...
!~~~>     OUTPUT ARGUMENTS:
!
!     ADYM (N by M array, double precision) -> vector of final adjoint states (at T->Tstart), i.e. solutions
!                  of the M first order adjoint final value problems
!
!     WM (P by 1 array, double precision) ->  solutions of the quadrature ODE systems
!
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   DOUBLE PRECISION, INTENT(INOUT) :: ADYM(N,M)
   DOUBLE PRECISION, INTENT(INOUT) :: WM(P,M)
   DOUBLE PRECISION, INTENT(INOUT) :: Tstart,Tend
   DOUBLE PRECISION, INTENT(INOUT) :: FwdAbsTol(N,M), FwdRelTol(N,M)
   DOUBLE PRECISION, INTENT(INOUT) :: AdjAbsTol(N+P,M), AdjRelTol(N+P,M)
...
! ADJ:
   ...
   IF (.NOT. Checkpointing) THEN
       ...
       DO i=1,M
          ...
          CALL RKINT_ADJ(N, ADYM(:,i), ADJ_RHS, JACV,                        &
                       P, WM(:,i), QUAD_RHS,                                 &
                       Tstart, Tend,                                         &
          ...
          !prepare the memory buffers for reuse
          CALL tapes_ReinitPointers
   ...
   DO i=1,M
       ...
       IF (AdjICNTRL(5) .EQ. 1) THEN
           ...
           CALL DY0DP(N,P,Tstart,ADYM(:,i),Wupdate)
           CALL WAXPY(P,1.d0,Wupdate,1,WM(:,i),1)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   IERR = Code
   PRINT * , &
     'Forced exit from RKINT_ADJDR_M due to the following error:' 
 ...
 END SUBROUTINE RKINT_ADJDR_M


END MODULE RKADJDRM_MODULE