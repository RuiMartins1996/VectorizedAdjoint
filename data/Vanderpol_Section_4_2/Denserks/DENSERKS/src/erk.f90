...
!~~~> Module containing the forward model integrator (subroutine RKINT)
MODULE RK_MODULE
...
!~~~> statistics regarding the work performed by the Runge-Kutta method
!~~~> these are the indices of the ISTATUS and RSTATUS vectors returned by RKINT
  INTEGER, PARAMETER :: Nfun=1, & !ISTATUS(Nfun) = number of ODE right-hand side evaluations 
			...
			Ntexit=1, & !RSTATUS(Ntexit) = Texit, the time corresponding to the
                                    !computed forward model solution upon return
			Nhexit=2, & !RSTATUS(Nhexit) = Hexit, last accepted step before exit
			Nhnew=3 !RSTATUS(Nhnew) = Hnew, last predicted step (not yet taken)
 CONTAINS

!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
SUBROUTINE RKINT(N, Y, RHS, Tstart, Tend, Nd, Nc,    &
           AbsTol, RelTol,                           &
           RCNTRL, ICNTRL, RSTATUS, ISTATUS, IERR)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!
!    Solves the system y'=F(t,y) using an explicit Runge-Kutta method.
!    The user can choose the desired RK method from a range of options.
!
...
!                  = vector of initial conditions (at T=Tstart)
!
...
!     RelTol(N), AbsTol(N) = user precribed forward model integrator relative and
!                            absolute tolerances
!
...
!        = 0 :    Disabled (use for forward model integration test runs; no data is stored)
...
!
!    Y(N) (N by 1 array, double precision)  -> Numerical solution at T
!
!    IERR (integer)    -> Reports on successfulness upon return:
!                    = 1 for success
!                    < 0 for error (value equals error code)
!
!    ISTATUS(20) (20 by 1 array, integer)
!    RSTATUS(20) (20 by 1 array, double precision)
!
...
!~~~>   DeltaMin is used as the initial time step if no time step is provided in RCNTRL(3)
...
!~~~>  For Vector tolerances (ICNTRL(1).EQ.0) the code uses AbsTol(1:N) and RelTol(1:N)
   IF (ICNTRL(1) == 0) THEN
      VectorTol = .TRUE.
      UplimTol  = N
   ELSE
...
!~~~>   The maximum number of steps admitted
   ...
   ELSE
      !error: the maximum number of time steps cannot be negative
...
!~~~>   Initialize the method code, order of accuracy and coefficients by calling a helper subroutine
...
!~~~> Buffering must be enabled for adjoint sensitivity analysis studies
   IF (ICNTRL(5) == 0) THEN
      !Only forward integration will be performed; no values will be stored
   ...
   ELSEIF (ICNTRL(5) == 1) THEN
      ...
      AdjSensitivity = .TRUE.
   ELSE
      !Invalid option; report the error and call error handler subroutine
...
!~~~>  Upper bound on the step size: (positive value)
   IF (RCNTRL(2) == ZERO) THEN
      !user didn't impose an upper bound on the time step size
...
!~~~>  Step size can be adapted s.t.  FacMin < Hnew/Hold < FacMax
   ...
   ELSEIF (RCNTRL(5) > ZERO) THEN
      ...
      FacMax = RCNTRL(5)
   ELSE
      !invalid value for FacMax: cannot be smaller than zero
      PRINT * , 'User-selected FacMax: RCNTRL(5)=', RCNTRL(5)
      CALL rk_ErrorMsg(-4,Tstart,ZERO,IERR)
      RETURN
   END IF

!~~~>   FacRej: Factor with which the time step is decreased after 2 succesive rejections
...
!~~~>   FacSafe: Safety Factor in the computation of new step size
   ...
   ELSEIF (RCNTRL(7) > ZERO) THEN
      !user requested value for FacSafe
      FacSafe = RCNTRL(7)
   ELSE
...
!~~~>   Make sure that Tstart <= Tend; this is required by RKINT
...
!~~~>  CALL Runge-Kutta core integrator
   CALL RK_Integrator(Y, RHS, Nd, Nc,            &
        Tstart, Tend, Texit,                     &
        AbsTol, RelTol,                          &
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  !total number of stages required for dense output
  rk_S = 16
  !order of accuracy
  rk_ELO = 8.d0
  !Runge-Kutta method code
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   IERR = Code
   PRINT * , &
     'Forced exit from RKINT due to the following error:' 

   SELECT CASE (Code)
    CASE (-1)
      PRINT * , '--> Improper value for maximal no of steps'
    CASE (-2)
      PRINT * , '--> Tapes are not yet allocated. Cannot store forward integration data.'
    CASE (-3)
    ...
    CASE (-12)
      PRINT * , '--> Improper option for memory buffers and checkpointing mechanisms. ', & 
                'Must choose either enabled (1) or disabled (0).'
    CASE DEFAULT
...
SUBROUTINE RK_Integrator (Y, RHS, Nd, Nc,        &
        Tstart, Tend, T,             &
        AbsTol, RelTol,                          &
!~~~> Integration parameters
        VectorTol, Max_no_steps,     &
...
!         T (double precision) : time value for which the solution has been computed
!                     (after successful return T=Tend)
!         Y (N by 1 array, double precision) :
!                    forward model state at T
...
!~~~> Input: integration parameters
   ...
   LOGICAL, INTENT(IN) :: Checkpointing, AdjSensitivity
...
!~~~> zero out the parts of Kbuf and Ktlm_buf that will not be used
   DO j=rk_S+1,s_Max
       CALL SET2ZERO(N,Kbuf(:,j))
   END DO

   IF (AdjSensitivity) THEN
      !Check if tapes are allocated only if adjoint sensitivity analysis is enabled
      CALL tapes_CheckAllocated(allocErr)
      IF (allocErr .NE. 1) THEN
          !tapes have not been allocated
...
!~~~> Compute the scaled norm of the error vector
   Err = rk_ErrorNorm ( N, Y, Ynew, Yerr, AbsTol, RelTol, VectorTol )
...
!~~~>  Check the error magnitude and adjust step size
      ...
      IF (AdjSensitivity) THEN
         ...
         IF (Checkpointing) THEN
            IF (mod(ISTATUS(Nacc),Nd) .EQ. 0) THEN
             !~~~> write a file checkpoint, storing enough data for a hot restart
              CALL tapes_WriteCheckpt(T,H,Y)
             !~~~> increment number of checkpoints & flush memory buffers (resets write pointer)
              CALL tapes_Flush
              Nc = Nc + 1
            END IF
         ...
         !~~~> write a new entry in the memory buffers
         !Kbuf(:,1:rk_S) = K
         DO j=1,rk_S
              CALL WCOPY(N,K(:,j),1,Kbuf(:,j),1)
         END DO
         CALL tapes_Write(T,H,Kbuf,Fcn0,Y)
   ...
   IF (AdjSensitivity) THEN
      ...
      ISTATUS(Nfun) = ISTATUS(Nfun) + 1
      !~~~> save last state in the tapes
      !~~~> this is needed in the continuous adjoint integration
      !Kbuf(:,1:rk_S) = K
      DO j=1,rk_S
           CALL WCOPY(N,K(:,j),1,Kbuf(:,j),1)
      END DO
      CALL tapes_Write(Tend,ZERO,Kbuf,Fcn0,Y)
...
!end of the RKINT integrator
END SUBROUTINE RKINT


SUBROUTINE rk_AllocateTapes(CheckptEnabled, MaxSize, N)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!~~~> Tape allocator -> calls tapes_Allocate
!~~~> This subroutine is callable from the driver program
!~~~> Allocates memory for the buffers (i.e. for storing the forward model trajectory)
! INPUTS:
!       LOGICAL CheckptEnabled --> File checkpoint mechanism enabled/disabled
...
!       INTEGER N --> Size of the forward model state
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  IMPLICIT NONE
  ...
  CALL tapes_Allocate(CheckptEnabled, MaxSize, N, s_Max)

END SUBROUTINE rk_AllocateTapes



SUBROUTINE rk_DeallocateTapes
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!~~~> Tape deallocator -> calls tapes_Deallocate
!~~~> This subroutine is callable from the driver program
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  CALL tapes_Deallocate

END SUBROUTINE rk_DeallocateTapes
