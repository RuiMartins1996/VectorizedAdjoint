...
!~~~> use the BLAS-like utility functions for vector computations
  USE UTILS
!~~~> use the coefficients for the Runge-Kutta methods defined in RKINT_TLM (avoids code duplication)
!~~~> maximum number of Runge-Kutta stages = s_Max = 16 (for DOPRI8(6) with dense output)
...
!           __|___
!             | b
  USE RKTLM_MODULE, ONLY : s_Max,A,b,c,d,e
  IMPLICIT NONE
  PUBLIC
  SAVE
!~~~>  Statistics on the work performed by the RK method
  INTEGER, PARAMETER :: adNrhs=1, & !ISTATUS(adNrhs) = number of first order adjoint ODE right-hand side evaluations
                        ...
                        adNrhsoa=6, & !ISTATUS(adNsoa) = number of second order adjoint ODE right-hand side evaluations
                        adNrhsp=7, & !ISTATUS(adNrhsp) = number of quadrature ODE right-hand side evaluations
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
SUBROUTINE RKINT_SOA(N, ADY, ADJ_RHS, JACV,          &
           AD2Y, SOA_RHS,                            &
           P, W2, QUAD2_RHS, DP,                     &
           Tstart, Tend,                             &
           AbsTol, RelTol,                           &
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!
!    Given an ODE y' = F(y,t), Tstart <= t <= Tend, RKINT_SOA simultaneously
!    integrates the corresponding continuous first and second order adjoint
!    final-value problems using an explicit Runge-Kutta method.
!
!    Either dense output or Hermite interpolation is used when interpolating
!    the forward and TLM solutions y(t) and dy(t) during the adjoint run.
!
...
!
!      P (integer)      = size of the quadrature ODE state
!
!      ADY (N by 1 array, double precision)
!                = vector of first order adjoint model final conditions (at T=Tend)
!
!      AD2Y (N by 1 array, double precision)
!                = vector of second order adjoint model final conditions (at T=Tend)
!
!      W2 (P by 1 array, double precision)
!                = Hessian-vector product (d2G/dp2)*DP (at T=Tend)
!
!      DP (P by 1 array, double precision)
!                = perturbation in the P parameters
!
!      ADJ_RHS (external) = name of the subroutine computing the RHS of
!                the first order adjoint ODE for a given adjoint state value.
...
!                       **Output:**
!                       DOUBLE PRECISION :: ADRHS1(N) !RHS of the first order adjoint ODE
!
...
!                    SUBROUTINE SOA_RHS(N,P,T1,Y1,DY1,DP,ADY1,AD2Y1,Soa)
!                       **Inputs:**
...
!                       **Output:**
!                       DOUBLE PRECISION :: Soa(N) !RHS of the SOA ODE
!
!      QUAD2_RHS = name(external) of the subroutine computing the RHS of the quadrature ODE
...
!             [Tstart,Tend]  = time range of integration; RKINT_SOA requires that Tstart <= Tend
!
!     RelTol(2*N+P by 1 array, double precision),
!     AbsTol(2*N+P by 1 array, double precision)
!                             = user precribed tolerances for the first order adjoint (RelTol(1:N),
!                               AbsTol(1:N)), second order adjoint variables (RelTol(N+1:2N),
!                               AbsTol(N+1:2N)), and quadrature variables (RelTol(2N+1:2N+P),
!                               AbsTol(2N+1:2N+P))
!
!    ICNTRL(20 by 1 array, double precision)    = integer input parameters
!    RCNTRL(20 by 1 array, double precision)    = real input parameters
!
!    For more information on ICNTRL and RCNTRL, see the "INPUT PARAMETERS" section below.
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!
!~~~>     INPUT PARAMETERS:
!
...
!
!    AD2Y (N by 1 array, double precision) = second order adjoint model state at T
!
!    ADY (N by 1 array, double precision) = first order adjoint model state at T
!
!    W2 (P by 1 array, double precision)
!                = if parameters are present and the integration completed successfully,
!                  the Hessian-vector product (G_pp)*DP
!
...
!    ISTATUS(6)  -> No. of SOA ODE RHS evaluations
!
...
!~~~>  Arguments
   INTEGER,          INTENT(INOUT) :: N, P
   DOUBLE PRECISION, INTENT(INOUT) :: ADY(N), AD2Y(N), W2(P), DP(P)
   DOUBLE PRECISION, INTENT(INOUT) :: Tstart,Tend
   DOUBLE PRECISION, INTENT(IN)    :: AbsTol(2*N+P),RelTol(2*N+P)
   INTEGER,          INTENT(IN)    :: ICNTRL(20)
   DOUBLE PRECISION, INTENT(IN)    :: RCNTRL(20)
   INTEGER,          INTENT(INOUT) :: ISTATUS(20)
   DOUBLE PRECISION, INTENT(INOUT) :: RSTATUS(20)
   INTEGER,          INTENT(OUT)   :: IERR
   EXTERNAL ADJ_RHS, JACV, SOA_RHS, QUAD2_RHS
...
!~~~>   The maximum number of steps admitted
   ...
   ELSEIF (ICNTRL(3) .EQ. 6) THEN
      ...
      rk_ELO = 8.d0
   ELSE
      PRINT * ,'Selected Runge-Kutta method not implemented: ICNTRL(3)=',ICNTRL(3)
      CALL rk_ErrorMsg(-2,Tstart,ZERO,IERR)
      RETURN
   END IF

   !Check the interpolation method that the user wants to employ
   IF ((FwdMethod .EQ. RK2) .OR. (FwdMethod .EQ. RK3) .OR. &
        &(FwdMethod .EQ. RK4)) THEN
       !for low-order methods, only Hermite interpolation is available
       IF (.NOT. ((ICNTRL(4) .EQ. 1) .OR. (ICNTRL(4) .EQ. 2))) THEN
            PRINT * ,'Wrong choice of interpolation for the selected ', & 
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   IERR = Code
   PRINT * , &
     'Forced exit from RKINT_SOA due to the following error:' 
   ...
   SELECT CASE (Code)
    ...
    CASE (-8)
      PRINT * , '--> User selected Tstart > Tend. ', &
                'RKINT_SOA requires that Tstart <= Tend.'
    CASE (-9)
      PRINT * , '--> Allocation error! Check available physical memory'
    CASE (-10)
      PRINT * , '--> Improper choice of interpolation strategy.'
    CASE (-11)
      PRINT * , '--> Invalid option for the parameters. ',& 
                'User must specify either ICNTRL(5) = 0 (no parameters) or ICNTRL(5) = 1 (P parameters).'
    CASE DEFAULT
...
!         AD2Y (N by 1 array, double precision) :
!                    second order adjoint model state at Tend
!         W2 (P by 1 array, double precision) :
...
!         SOA_RHS (external) : pointer to (user-supplied) subroutine that evaluates
!                          the right-hand side of the second order adjoint model ODE
!         QUAD2_RHS (external) : pointer to (user-supplied) subroutine that evaluates
...
!
!         For more information on RSTATUS and ISTATUS, see the comments describing the RKINT_TLM subroutine
!
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~


   IMPLICIT NONE
   EXTERNAL ADJ_RHS, SOA_RHS, QUAD2_RHS
!~~~> Input: the initial condition at Tend; Output: the solution at T
   DOUBLE PRECISION, INTENT(INOUT) :: ADY(N)
   DOUBLE PRECISION, INTENT(INOUT) :: AD2Y(N)
   DOUBLE PRECISION, INTENT(INOUT) :: W2(P), DP(P)
!~~~> Input: integration interval
   DOUBLE PRECISION, INTENT(INOUT) :: Tstart,Tend
!~~~> Output: time at which the solution is returned (T=Tstart if success)
...
!~~~> New adjoint solution
   DOUBLE PRECISION :: ADYnew(N), AD2Ynew(N), W2new(P)
!~~~> Kadj and K2adj = the stages for the integration of the adjoint equations
!~~~> Ktau = stages for the quadrature ODE integration
   DOUBLE PRECISION :: Kadj(N,rk_S)
   DOUBLE PRECISION :: K2adj(N,rk_S)
   DOUBLE PRECISION :: Kw2(P,rk_S)

   DOUBLE PRECISION :: H, Hnew, Fac, Tau
   DOUBLE PRECISION :: Err, Yval(N), DYval(N)
   DOUBLE PRECISION :: ADYerr(N), AD2Yerr(N), W2err(P)
!~~~> Extended vectors for local error estimation
   DOUBLE PRECISION :: ADYExt(2*N+P), ADYExtnew(2*N+P), ADYExterr(2*N+P)

   DOUBLE PRECISION :: Jac0(N), Jac(N), Soa0(N), Soa(N)
   DOUBLE PRECISION :: Quad0(P), Quad(P)
...
!~~~> Check if the tapes have been allocated (user has to take care of this in the driver)
!~~~> Perform check for both FWD and TLM tapes using tlm_tapes_CheckAllocated
!~~~> The user has to take care of this in the driver by calling rk_AllocateTapes
! (inside module RKTLM_MODULE)
...
AdjUntilAccepted: DO
   ...
   CALL rk_InterpolateFwd(T, Yval, DYval)

   CALL ADJ_RHS(N,T,Yval,ADY,Jac0)
   CALL WSCAL(N,-1.d0,Jac0,1)
   ISTATUS(adNrhs) = ISTATUS(adNrhs) + 1

   CALL SOA_RHS(N,P,T,Yval,DYval,DP,ADY,AD2Y,Soa0)
   CALL WSCAL(N,-1.d0,Soa0,1)
   ISTATUS(adNrhsoa) = ISTATUS(adNrhsoa) + 1

   IF (Parameters) THEN
       CALL QUAD2_RHS(N,P,T,Yval,DYVal,DP,ADY,AD2Y,Quad0)
       CALL WSCAL(P,-1.d0,Quad0,1)
...
AdjStage: DO istage = 1, rk_S
     ...
     IF (istage .EQ. 1) THEN
         ...
         !Soa = Soa0
         CALL WCOPY(N,Soa0,1,Soa,1)

         IF (Parameters) THEN
             !Quad = Quad0
             CALL WCOPY(P,Quad0,1,Quad,1)
         END IF
     ...
     ELSE
         ...
         !AD2Ynew = AD2Y
         CALL WCOPY(N,AD2Y,1,AD2Ynew,1)

         IF (Parameters) THEN
             !W2new = W2
             CALL WCOPY(P,W2,1,W2new,1)
         END IF
         ...
         DO j = 1, istage-1
             ...
             !AD2Ynew = AD2Ynew + H*A(istage,j)*K2adj(:,j)
             CALL WAXPY(N,H*A(istage,j),K2adj(:,j),1,AD2Ynew,1)

             IF (Parameters) THEN
                 !TAUnew = TAUnew + H*A(istage,j)*Kw2(:,j)
                 CALL WAXPY(P,H*A(istage,j),Kw2(:,j),1,W2new,1)
             END IF
         ...
         CALL rk_InterpolateFwd(Tau,Yval,DYval)

         CALL ADJ_RHS(N,Tau,Yval,ADYnew,Jac)
         CALL WSCAL(N,-1.d0,Jac,1)
         ISTATUS(adNrhs) = ISTATUS(adNrhs) + 1

         CALL SOA_RHS(N,P,Tau,Yval,DYval,DP,ADYnew,AD2Ynew,Soa)
         CALL WSCAL(N,-1.d0,Soa,1)
         ISTATUS(adNrhsoa) = ISTATUS(adNrhsoa) + 1

         IF (Parameters) THEN
             CALL QUAD2_RHS(N,P,Tau,Yval,DYVal,DP,ADYnew,AD2Ynew,Quad)
             CALL WSCAL(P,-1.d0,Quad,1)
             ISTATUS(adNrhsp) = ISTATUS(adNrhsp) + 1
         END IF
     ...
     !K2adj(:,istage) = Soa
     CALL WCOPY(N,Soa,1,K2adj(:,istage),1)

     IF (Parameters) THEN
          !Kw2(:,istage) = Quad
          CALL WCOPY(P,Quad,1,Kw2(:,istage),1)
     END IF
...
!~~~>  Compute the new solution
   ...
   DO j=1,rk_S
        !ADYnew = ADYnew + H*b(j)*Kadj(:,j)
        CALL WAXPY(N,H*b(j),K2adj(:,j),1,AD2Ynew,1)
   END DO
   ...
   IF (Parameters) THEN
      ...
      CALL WCOPY(P,W2,1,W2new,1)
      DO j=1,rk_S
           !W2new = W2new + H*b(j)*Kw2(:,j)
           CALL WAXPY(P,H*b(j),Kw2(:,j),1,W2new,1)
      END DO
...
!~~~>  Compute the error estimation
   ...
   !AD2Yerr = 0.d0
   CALL SET2ZERO(N,AD2Yerr)
   DO j=1,rk_S
        !AD2Yerr = AD2Yerr + H*e(j)*K2adj(:,j)
        CALL WAXPY(N,H*e(j),K2adj(:,j),1,AD2Yerr,1)
   END DO

   IF (Parameters) THEN

      !W2err = 0.d0
      CALL SET2ZERO(P,W2err)
      DO j=1,rk_S
           !W2err = W2err + H*e(j)*Kw2(:,j)
           CALL WAXPY(P,H*e(j),Kw2(:,j),1,W2err,1)
      END DO

   END IF


   CALL WCOPY(N,ADY,1,ADYExt(1:N),1)
   CALL WCOPY(N,AD2Y,1,ADYExt(N+1:2*N),1)

   CALL WCOPY(N,ADYnew,1,ADYExtnew(1:N),1)
   CALL WCOPY(N,AD2Ynew,1,ADYExtnew(N+1:2*N),1)

   CALL WCOPY(N,ADYerr,1,ADYExterr(1:N),1)
   CALL WCOPY(N,AD2Yerr,1,ADYExterr(N+1:2*N),1)

   IF (Parameters) THEN
      CALL WCOPY(P,W2,1,ADYExt(2*N+1:2*N+P),1)
      CALL WCOPY(P,W2new,1,ADYExtnew(2*N+1:2*N+P),1)
      CALL WCOPY(P,W2err,1,ADYExterr(2*N+1:2*N+P),1)
      Err = rk_ErrorNorm ( 2*N+P, ADYExt, ADYExtnew, ADYExterr, AbsTol, RelTol, VectorTol )
   ELSE
      Err = rk_ErrorNorm ( 2*N, ADYExt(1:2*N), ADYExtnew(1:2*N), &
                  ADYExterr(1:2*N), AbsTol(1:2*N), RelTol(1:2*N), VectorTol )
   END IF
...
!~~~>  Check the error magnitude and adjust step size
   ...
   IF ( (Err <= ONE).OR.(H <= Hmin) ) THEN  !~~~> Accept step
      ...
      !AD2Y = AD2Ynew
      CALL WCOPY(N,AD2Ynew,1,AD2Y,1)
      IF (Parameters) THEN
          !W2 = W2new
          CALL WCOPY(P,W2new,1,W2,1)
      END IF
      !update current time
      T = T - H
...
SUBROUTINE rk_DenseOutput4(Model, T, Yval)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
! Inputs:
!        Model (integer) :
!                = FWD : interpolate the forward model solution
!                = TLM : interpolate the tangent linear model solution
!        T (double precision) : time at which a fwd/tlm solution approximation
!                               is required
!
! Outputs:
!        Yval (N by 1 array, double precision) : interpolated solution at T
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  IMPLICIT NONE
...
!~~~> perform a tape lookup
  IF (Model .EQ. FWD) THEN
      CALL tlm_tapes_LookupDense(T,Tp,Hp,Kp,Yp)
  ELSE IF (Model .EQ. TLM) THEN
      CALL tlm_tapes_LookupDenseTLM(T,Tp,Hp,Kp,Yp)
  END IF

  theta = (T - Tp) / Hp
  temp1 = theta**2 * (theta-1.d0)**2
  temp2 = theta**2 * (3.d0 - 2.d0*theta)

!~~~> we use the dense output formulas from Hairer and Wanner's textbook
  b_theta(1) = temp2*b(1) + theta*(theta-1.d0)**2 - 5.d0*temp1*(2558722523.d0 - 31403016.d0*theta)/11282082432.d0
  b_theta(2) = 0.d0
  ...
  b_theta(7) = theta**2 * (theta-1.d0) + 10.d0*temp1*(7414447.d0 - 829305.d0*theta)/29380423.d0
...
SUBROUTINE rk_DenseOutput6(Model, T, Yval)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  DO i=1,rk_S
     bb(i) = dot_product(b_theta(i,:),theta)
  END DO
...
SUBROUTINE rk_DenseOutput7(Model, T, Yval)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!                = FWD : interpolate the forward model solution
!                = TLM : interpolate the tangent linear model solution
!        T (double precision) : time at which a fwd/tlm solution approximation is required
!
! Outputs:
!        Yval (N by 1 array, double precision) : interpolated solution at T
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  INTEGER :: j,jj
  DOUBLE PRECISION :: Yc(N), Ycp1(N), Kc(N,s_Max)
  DOUBLE PRECISION :: Theta, Theta1, Tc, Hc
  DOUBLE PRECISION :: Cont(N,0:7)

  !do tape lookup
  IF (Model .EQ. FWD) THEN
      CALL tlm_tapes_LookupDense(T,Tc,Hc,Kc,Yc,Ycp1)
  ELSE IF (Model .EQ. TLM) THEN
      CALL tlm_tapes_LookupDenseTLM(T,Tc,Hc,Kc,Yc,Ycp1)
  END IF
...
SUBROUTINE rk_Hermite3(Model, T, Yval)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!                = FWD : interpolate the forward model solution
!                = TLM : interpolate the tangent linear model solution
!        T (double precision) : time at which a fwd solution approximation is required
!
! Output:
!        Yval (N by 1 array, double precision) : interpolated solution at T
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   IF (Model .EQ. FWD) THEN
       CALL tlm_tapes_LookupHermite(T,Ta,Tb,Ya,Yb,Ja,Jb)
   ELSE IF (Model .EQ. TLM) THEN
       CALL tlm_tapes_LookupHermiteTLM(T,Ta,Tb,Ya,Yb,Ja,Jb)
   END IF
...
SUBROUTINE rk_Hermite5(Model, T, Yval)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
SUBROUTINE rk_InterpolateFwd(T,Yval,DYval)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
! This subroutine interpolates the forward model and TLM solutions at a given time T
...
!
! Input:
!        T (double precision) : time at which the solution approximations are required
!
! Output:
!        Yval (N by 1 array, double precision) : interpolated forward solution at T
!        DYval (N by 1 array, double precision) : interpolated TLM solution at T
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

  IMPLICIT NONE
  DOUBLE PRECISION, INTENT(INOUT) :: T
  DOUBLE PRECISION, INTENT(OUT) :: Yval(N), DYval(N)
  ...
  IF ((FwdMethod .EQ. RK2) .OR. &
        ...
        IF (IntMethod .EQ. 1) THEN
             CALL rk_Hermite3(FWD,T,Yval)
             CALL rk_Hermite3(TLM,T,DYval)
        ELSE IF (IntMethod .EQ. 2) THEN
             CALL rk_Hermite5(FWD,T,Yval)
             CALL rk_Hermite5(TLM,T,DYval)
        END IF
  ...
  ELSE IF (FwdMethod .EQ. RK5) THEN
        ...
        ELSE IF (IntMethod .EQ. 2) THEN
             ...
             CALL rk_Hermite5(TLM,T,DYval)
        ELSE
             CALL rk_DenseOutput4(FWD,T,Yval)
             CALL rk_DenseOutput4(TLM,T,DYval)
        END IF
  ...
  ELSE IF (FwdMethod .EQ. RK6) THEN
        ...
        ELSE IF (IntMethod .EQ. 2) THEN
             ...
             CALL rk_Hermite5(TLM,T,DYval)
        ELSE
             CALL rk_DenseOutput6(FWD,T,Yval)
             CALL rk_DenseOutput6(TLM,T,DYval)
        END IF
  ...
  ELSE IF (FwdMethod .EQ. RK8) THEN
        ...
        ELSE IF (IntMethod .EQ. 2) THEN
             ...
             CALL rk_Hermite5(TLM,T,DYval)
        ELSE
             CALL rk_DenseOutput7(FWD,T,Yval)
             CALL rk_DenseOutput7(TLM,T,DYval)
        END IF

  END IF

END SUBROUTINE rk_InterpolateFwd


END SUBROUTINE RKINT_SOA


!END OF MODULE
END MODULE RKSOA_MODULE