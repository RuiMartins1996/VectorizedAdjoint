...
PROGRAM MAIN
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!~~~> Driver for the second order adjoint of the scaled Van der Pol system.
!~~~> Two second order adjoint model integrations are performed (with different
! initial conditions).
!~~~> epsilon = 0.01d0
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  INTEGER, PARAMETER :: M = 2 !Two SOA model integrations will be performed
  INTEGER, PARAMETER :: P = 0 !No parameters (sensitivity w.r.t. initial conditions)
  ...
  DOUBLE PRECISION :: Y(N), DY(N), ADY(N,M), AD2Y(N,M)
  DOUBLE PRECISION :: DP(P,M), W(P,M)

  INTERFACE

     SUBROUTINE INTEGRATE(N, M, P, Y, DY, DP, ADY,        &
                          AD2Y, W2, TIN, TOUT,            &
                          ICNTRL_U, RCNTRL_U,             &
                          ISTATUS_U, RSTATUS_U, IERR_U,   &
                          adICNTRL_U, adRCNTRL_U,         &
                          adISTATUS_U, adRSTATUS_U, adIERR_U)
  ...
  !setup models' initial conditions
  Y(1) = 2.d0
  Y(2) = 0.0d0
...
! Set all integrator parameters to zero
  ...
  CALL INTEGRATE(N, M , P, Y, DY, DP, ADY, AD2Y, W, t0, tF, &
...
! --- PRINT FIRST ORDER ADJOINT SOLUTION
  PRINT *,'First order adjoint solution:'
  PRINT *,'ADY(1,:) = ',  ADY(1,1:M)
  PRINT *,'ADY(2,:) = ',  ADY(2,1:M)

! --- PRINT SECOND ORDER ADJOINT SOLUTION
  PRINT *,'Second order adjoint solution:'
  PRINT *,'AD2Y(1,:) = ',  AD2Y(1,1:M)
  PRINT *,'AD2Y(2,:) = ',  AD2Y(2,1:M)
  PRINT *, '****************'
...
SUBROUTINE INTEGRATE(N, M, P, Y, DY, DP, ADY, AD2Y, W2, TIN, TOUT, &
  ...
  adICNTRL_U, adRCNTRL_U, adISTATUS_U, adRSTATUS_U, adIERR_U)
   ...
   DOUBLE PRECISION :: adATOL(2*N+P,M), adRTOL(2*N+P,M)
...
!~~~> Problem is not parameter-dependent
   adICNTRL(5) = 0
...
!~~~> Enable checkpointing and memory buffering
   ...
   CALL  RKINT_SOADR_M(N, M, ADY, ADJRHS, JACV,                     &
...
!==============================================
  ...
  g_adj1(1) = g_ady1(2)*((1.d0+2.d0*y1(1)*y1(2))/eps)+g_y1(2)*(2*y1(1)*ady1(2)/eps)+g_y1(1)*(2*y1(2)*ady1(2)/eps)
  g_adj1(2) = (-(g_ady1(2)*((1.d0-y1(1)**2)/eps)))-g_ady1(1)+g_y1(1)*(2*y1(1)*ady1(2)/eps)
