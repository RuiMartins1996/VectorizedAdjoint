...
public:
    ...
    AadDataNew(const int Nin, const int Nout, const int Npar, System system)
    {
    ...
    AadDataNew(AadDataNew const &) = delete;
    AadDataNew &operator=(AadDataNew const &) = delete;

    // Move assignment
    AadDataNew &operator=(AadDataNew &&other)
    {
        if (this != &other)
        {
    ...
    // Move Constructor
    AadDataNew(AadDataNew &&other) noexcept
    {
        p_aad_funcs = std::move(other.p_aad_funcs);
        ws = std::move(other.ws);
        input_args = std::move(other.input_args);
...
template <class System>
std::unique_ptr<AadDataNew> AadDataFactory(const int Nin, const int Nout, const int Npar,
                                           System system)
{
    // Implicit move operation into the variable that stores the result.
    // AadDataNew aad(Nin, Npar, stepper, system);
    return std::make_unique<AadDataNew>(Nin, Nout, Npar, system);
};
